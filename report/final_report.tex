...
\begin{abstract}
Tool-using language agents must often decide whether to act immediately or pay for information that may improve a later action. In multi-tool settings, this splits into two questions: whether to ask and which tool family to ask. This report extends the midway study of \texttt{voi\_memory\_v1}, a family-scoped memory variant tested in a cloneable ToyTextMDP environment with \textsc{scan}, \textsc{verify}, and \textsc{choose} actions. Since the midway report, I added scaled dual-battery runs, repaired reference generation, stricter receipt checks, and a v4 aggressive-stress task ecology designed to expose scan/verify conflict. The final receipt-safe v4 seed-7 battery does not support a stable which-tool improvement: the locked \texttt{voi\_memory\_v1} comparison slightly worsens choose-swallow and family-mismatch rates from reference to stress, and verify usage drops. Both reference and stress blocked-step audits route all blocked rows to task pressure. The main contribution is therefore not a positive tool-choice claim, but a controlled diagnostic ladder that makes the claim testable and rejects it cleanly under the current task ecology.
\end{abstract}

\section{Introduction}

Language agents are increasingly evaluated as systems that reason, call tools, receive observations, and revise behavior over time \citep{yao2023react,shinn2023reflexion,liu2024agentbench,jimenez2024swebench}. A central decision in these systems is epistemic: should the agent act now, or should it ask for more information? In a multi-tool setting, there is a second layer. If asking is useful, which information source should be queried?

The midway report narrowed the original broad proposal into a controlled diagnostic problem. Rather than claiming that memory already solved tool choice, it asked whether a family-scoped memory variant, \texttt{voi\_memory\_v1}, could make a meaningful which-tool problem observable in a small, cloneable text environment. That report showed progress on a 10-task smoke batch: nonzero tool-choice opportunity appeared, but tool precision stayed at ceiling and blocked rows under \texttt{voi\_memory\_v1} still routed to task pressure. The safest claim was whether-to-ask progress, not which-tool improvement.

This final report extends that arc with the missing large-scale and confirmation evidence. I ran receipt-safe dual batteries, repaired reference packs when preflight failed, and tested a stronger v4 stress ecology. The result is a stable negative finding: under the current task profiles, \texttt{voi\_memory\_v1} does not improve the locked which-tool comparison. This is still useful. It turns a vague agent-memory story into a falsifiable diagnostic ladder with explicit claim boundaries, receipt gates, and route decisions.

The paper makes three contributions. First, it defines the task and memory machinery precisely enough for a classmate to inspect it. Second, it reports the post-midway experimental ladder, separating offline diagnostics from receipt-safe live evidence. Third, it shows that the current bottleneck is best explained by \texttt{task\_profile / task\_pressure}, not by an accepted need to edit the gate, memory, or selector.
...
The receipt layer is a reproducibility gate rather than an analysis tool. A run must satisfy \texttt{run\_completed=true}, \texttt{jsonl\_integrity=pass}, \texttt{task\_id\_pairing=pass}, and \texttt{safe\_for\_interpretation=true}. Only then can the diagnosis scripts read the JSONL, compare the selected family to the oracle family, and route the battery. Route A would continue confirmation, Route B is a stable negative or task-pressure review, and Route C is reserved for negative inversion that would justify layer-level diagnosis. In the final battery, the route is B.
...
All experiments use internally generated tasks rather than a public benchmark. This limits external realism but makes attribution tight, unlike broader agent suites such as AgentBench and SWE-bench \citep{liu2024agentbench,jimenez2024swebench}. Each live battery compares \texttt{full} (baseline), \texttt{selector\_penalty} (a non-memory anti-ask variant), and \texttt{voi\_memory\_v1} (family-scoped lessons). A battery is interpreted only if both packs pass four receipt gates: run completion, JSONL integrity, task-id pairing, and safety for interpretation.
...
The important methodological choice is that failures are preserved rather than overwritten. A reference pack that fails combined preflight is not frozen, hashed, or used in live evidence. A stress-only separation is not upgraded into a confirmation result. This accounting is what makes the final negative result interpretable: the paper does not conclude ``no effect'' because one exploratory probe was weak; it concludes that the live evidence ladder repeatedly failed to turn stronger task pressure into a stable locked memory advantage.
...
The final v4 seed-7 battery is the current route authority. Both reference and stress packs pass the receipt layer. The battery summary routes to B: no trigger-good finding on the locked primary mode, no negative inversion, and a task-pressure/stable-negative review as the next action. This is a clean negative result, not a failed run.
...
The most important row is \texttt{voi\_memory\_v1}. In the reference pack, choose-swallow is 0.5291 and selected-vs-oracle family mismatch is 0.5291. In the v4 stress pack, these slightly worsen to 0.5376. Verify usage drops from 0.2063 to 0.0860. Thus the stronger v4 task ecology did not turn memory into a stable which-tool improvement. It instead preserved the same broad failure shape: the system often reaches tool-choice opportunities but still swallows them by choosing directly or by not using verify when the oracle says verify matters.

There are two useful secondary observations. First, the stress pack does expose wrong-tool surface in \texttt{full} and \texttt{selector\_penalty}, where family mismatch rises above reference. This means the v4 stress design did create sharper scan/verify conflict. Second, that surface disappears in \texttt{voi\_memory\_v1} not because verify succeeds, but because the mode still has high choose-swallow and lower verify usage. The negative result is therefore more informative than the midway smoke: the task ecology got sharper, but the locked memory mode still did not convert that pressure into reliable tool-family choice.
...
Figure~\ref{fig:case} shows the distinction that motivated the additional analysis. The denominator is not all environment steps; it is the subset where oracle replay says a tool family is relevant and the action space still contains meaningful tool/choose alternatives. Within that subset, direct choice is counted as choose-swallow, not as a tool-family success. This is why high opportunity counts can coexist with a negative route.

This is why the claim boundary matters. The accepted claim remains narrow: \texttt{voi\_memory\_v1} improved whether-to-ask behavior in the earlier single-tool setting. The final batteries do not establish stable which-tool improvement, independent memory lift, or a reason to edit the ask gate, memory, or selector.
...
\paragraph{Why Route B is the right conclusion.}
Route B is stricter than saying the experiment ``did not work.'' It means the run is valid, the JSONL can be interpreted, no negative inversion justifies a layer switch, and the locked primary comparison does not support a positive which-tool claim. This matters because the seed-11 stress-only probe briefly looked encouraging: family mismatch improved inside the stress pack. The repaired seed-11 reference confirmation and the v4 seed-7 live battery prevent that local pattern from being overpromoted. In other words, the framework rejects the claim for the same reason it could have accepted it: the comparison is now visible and receipt-safe.
...
The project is small, but the negative result is disciplined. Many agent papers report headline scores on broad tasks; here the contribution is diagnostic control. The system can say when an offline pack is not eligible, when a live run is receipt-safe, when a stress-only signal is not enough, and when a route should stop. This style follows recent concerns that evaluation choices can change conclusions about system quality \citep{levtsov2025confidence}. For this project, the receipt and battery summary are not clerical details; they are the guardrails that prevent an attractive local pattern from becoming an unsupported claim.
...
Structured memory did not improve the locked which-tool comparison under the current task ecologies. The contribution is a receipt-safe diagnostic ladder: it exposes tool-choice opportunities, rejects an unsupported positive claim, and points the next review toward task pressure rather than gate, memory, or selector changes.
