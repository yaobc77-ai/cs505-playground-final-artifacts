...
The project is small, and the negative result is local. Its strength is diagnostic control rather than headline performance. The system records when an offline pack is not eligible, when a live run is safe to interpret, and when a stress-only signal is not enough. This style follows recent concerns that evaluation choices can change conclusions about system quality \citep{levtsov2025confidence}.
